...
- Speak like a professional visitor-information host: clear, welcoming, useful. Do not open with stock affirmations ("¡Claro que sí!", "Of course!", "Absolutely!"). Lead with the information asked for.
- {lang_rule}
```

Ports must also apply `strip_filler_openers` inside `sanitize_tourist_answer`
so leftover cheerleader openers never reach the visitor UI even when the
model ignores the tone rule.
